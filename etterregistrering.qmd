...
```{python}
df.stillingskategori = df.stillingskategori.map(dict(STILLING="Registrering av formidling", FORMIDLING="Etterregistrering av formidling"))
```
...
def plot_andel_etterregistrering(df, kolonne, kolonne_navn):
    antall_etterregistrering = (
        df[df.stillingskategori == "Etterregistrering av formidling"].groupby(kolonne).antall.sum()
    )
...
```{python}
#| title: Totalt antall registreringer vs andel etterregistrering
fig = go.Figure(
    data=[
        go.Scatter(
...
fig = fig.update_layout(
    xaxis_title="Totalt antall registreringer",
...
```{python}
#| title: Antall etterregistreringer
fig = go.Figure(
    data=[
        go.Histogram(
...
fig = fig.update_layout(
    xaxis_title="Antall etterregistreringer",
    yaxis_title="Antall kontorer",
)
fig.show()
```
...
```{python}
#| title: Antall registreringer av formidling
fig = go.Figure(
    data=[
        go.Histogram(
...
fig = fig.update_layout(
    xaxis_title="Antall registreringer",
    yaxis_title="Antall kontorer",
)
fig.show()
```