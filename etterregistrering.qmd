...
query = f"""
    select
        ...
        count(*) as antall
    from `rekrutteringsbistand_kandidat_pg15.utfallsendring` u
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandidat` v on u.veilkandidat_db_id = v.db_id
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandliste` vk on v.veilkandliste_db_id = vk.db_id
...
query = f"""
    select
        FORMAT_DATETIME('%Y-%m', lagt_til_tidspunkt) as tidspunkt_utfall_yearmonth,
        vk.stillingskategori,
        count(*) as antall
    from `rekrutteringsbistand_kandidat_pg15.formidlingavusynligkandidat` fk
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandliste` vk on fk.veilkandliste_db_id = vk.db_id
...
query = f"""
    ...
    from `rekrutteringsbistand_statistikk_pg15.kandidatutfall` u
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandliste` l on u.stillingsid = l.stilling_id
