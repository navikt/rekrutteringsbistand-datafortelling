...
query = f"""
    with unik_synlig_kandidat_presentert_per_liste as (
        select distinct aktor_id, uuid from `rekrutteringsbistand_kandidat_pg15.utfallsendring` u
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandidat` k on k.db_id = u.veilkandidat_db_id
        where (utfall = 'PRESENTERT' or utfall = 'FAATT_JOBBEN')
        and tidspunkt >= '2022-05-11T10:49:00'
    )
...
query = f"""
    with unik_kandidat_delt_cv_per_liste as (
        select distinct aktor_id, veilkandliste_db_id from `rekrutteringsbistand_kandidat_pg15.utfallsendring` u
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandidat` k on k.db_id = u.veilkandidat_db_id
...
query = f"""
    with unik_kandidat_delt_cv_per_liste as (
        select distinct aktor_id, veilkandliste_db_id, tidspunkt from `rekrutteringsbistand_kandidat_pg15.utfallsendring` u
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandidat` k on k.db_id = u.veilkandidat_db_id
        where sendt_til_arbeidsgivers_kandidatliste = true
        and tidspunkt >= '2022-05-11T10:49:00'
    )
...
query = f"""
    with unik_synlig_kandidat_presentert_per_liste as (
        select distinct aktor_id, uuid, tidspunkt from `rekrutteringsbistand_kandidat_pg15.utfallsendring` u
        inner join `rekrutteringsbistand_kandidat_pg15.veilkandidat` k on k.db_id = u.veilkandidat_db_id
        where (utfall = 'PRESENTERT' or utfall = 'FAATT_JOBBEN')
        and tidspunkt >= '2022-05-11T10:49:00'
    )
